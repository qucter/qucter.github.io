...
function serachTree() {
    ...
    $("#search").on("input", function(e){
        ...
        要求获取 inpiut 输入框的内容，事件是输入，监听者是id:search
        要获取值应选择target，secarch未赋初始值所以一直是undefined
        */

        // 获取 inpiut 输入框的内容
        var inputContent = e.target.value;
        //var xx=e.target.id
        //var xx=e.currentTarget;
        //alert(inputContent);
...
//点击目录文章滑动并且在文章滑动过的位置，导航都标记已读的状态
$(document).ready(function () {
...
function pjaxLoad(){
    ...
    // 页面初始化子目录收缩
    $("a:contains('图片')").siblings("ul").slideUp(1);
    $("a:contains('测试')").siblings("ul").slideUp(1);
    $("a:contains('杂谈')").siblings("ul").slideUp(1);
    $("a:contains('美食笔记')").siblings("ul").slideUp(1);
    $("a:contains('新建文件夹')").siblings("ul").slideUp(1);
    $("a:contains('笔记')").siblings("ul").slideUp(1);
    $("a:contains('杂图)").siblings("ul").slideUp(1);
    $(".menu-item-link")

}

$(".menu-item-link").on("click", function(e){
     document.getElementById("lable-t").innerText = "主页";
    });
$(".tag-css").on("click", function(e){
    var a=e.target.innerText;
    alert(a);
     document.getElementById("lable-t").innerText = a;
    });
